...
object LogSQL {
  ...
  def main(args: Array[String]) {
    ...
    // the set part is some hacky workaround to be used in Windows env. (for issue)
    
    val ssc = new StreamingContext(conf, Seconds(1))  // batch size 1s
    
    
    setupLogging()  // filter out log spam
    ...
    requests.foreachRDD((rdd, time) => {
      ...
      // In Scala we can import anywhere, not just at the top as in Java
      ...
      // Record is a case class defined at the bottom
      ...
      requestsDataFrame.write.json(s"DFtoJSON/$now")
