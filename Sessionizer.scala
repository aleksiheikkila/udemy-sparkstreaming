...
object Sessionizer {
   ...
   */
  // data type for sateful info we want to keep track of
  // clickstream is a list of urls the ip visited (ip is the key)
  ...
  // Option can be Some or None... String there is the IP
  def trackStateFunc(batchTime: Time, 
      ip: String, url: Option[String], 
    ...
    // list to 10 items
    // :+ to append
    // take(10) to cap len to 10 to keep it short... for educ purposes mostly :)
  ...
  def main(args: Array[String]) {
    ...
    // session timeout value of 30 minutes (no session can last more than 30mins in this case).
